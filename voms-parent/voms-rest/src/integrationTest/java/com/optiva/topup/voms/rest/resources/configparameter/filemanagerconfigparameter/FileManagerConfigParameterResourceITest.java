...
class FileManagerConfigParameterResourceITest extends BaseWebIntegrationTest {

  private static final String ROOT_ENTITY_NAME = "fileManagerConfigParameters";
  ...
  void givenRightPermissionsAndAuthenticatedShouldSaveFine() throws Exception {
    ...
    when(tokenStore.readAuthentication(any(OAuth2AccessToken.class)))
        .then(invocation -> getOauthTestAuthenticationWithRoles(
            GuiUserAuthoritiesHelper.FILE_MANAGER_CONFIG_PARAMETER));
  ...
  private FileManagerConfigParameter getConfigParameter() {
