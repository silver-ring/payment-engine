...
class ChangePasswordConfirmation extends React.Component {
  ...
  componentWillMount() {
    ...
    const {changePasswordToken} = this.props.match.params;

    const url = "changePasswordConfirmation/checkToken";

    const json = {"changePasswordToken": changePasswordToken};
  ...
  componentDidMount() {
    ...
    ValidatorForm.addValidationRule('isPasswordMatch', (value) => {
      ...
      return value === json["newPassword"];
    });
  }

  handleFormSubmit = (e) => {

    const {changePasswordToken} = this.state;

    if (!this.validationForm.isFormValid(false)) {
      return;
    }

    const url = "changePasswordConfirmation";

    const json = convertFormToJson(this.form);
    json["changePasswordToken"] = changePasswordToken;
  ...
  render = () => {
    ...
    const {changePasswordToken, alertMessage, loading, finished} = this.state;
            ...
            <main className={classes.layout}>
              <Paper className={classes.paper}>
                ...
                <Typography variant="body1" gutterBottom>
                  You Password Successfully Changed, Please click finish to be
...
export default withStyles(styles)(ChangePasswordConfirmation);
