...
import ModifyBlockedVoucherAdmin from "./ModifyBlockedVoucherAdmin";
import restResourceInstance from "../../../axios/RestResourceInstance";

class BlockedVoucherSearchResultAdmin extends React.Component {
  ...
  handleRefresh = () => {
    const successMessage = "Save Blocked Voucher Success";
    const successAlert = (
  ...
  render = () => {
    ...
    const activateLink = halfred.parse(data).link("activate")["href"];
    const voucherDetailsLink = halfred.parse(data).link(
        "blockedVoucherDetails")["href"];
          ...
          <Card>
            ...
            <CardActions style={{float: 'right'}}>
              <Button color={"primary"} variant={"contained"}
                      disabled={loading}
                      onClick={() => this.handleOnAction(activateLink)}>
                {loading ?
                    <CircularProgress size={20}/>
                    :
                    "Activate"
...
export default BlockedVoucherSearchResultAdmin;
